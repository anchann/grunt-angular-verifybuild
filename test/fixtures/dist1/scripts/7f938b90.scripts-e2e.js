...
var angtsApp = angular.module('angtsApp', ['angtsTemplates']).config([
    '$routeProvider',
    function ($routeProvider) {
...
angtsApp.controller('TasksController', [
  '$scope',
  'taskService',
...
var TaskService = function () {
    ...
    TaskService.prototype.get = function () {
      return this.$http.get(this.config.BACKEND_SERVER + '/tasks.json').then(function (response) {
        var rawTasks = response.data;
        return _.map(rawTasks, function (rawTask) {
          return new Task(rawTask.title);
        });
      });
    };
    return TaskService;
  }();
var angtsAppDev = angular.module('angtsAppDev', [
    'angtsApp',
    'ngMockE2E'
  ]);
angtsAppDev.run([
  ...
  function ($httpBackend, config) {
    ...
    $httpBackend.whenGET(config.BACKEND_SERVER + '/tasks.json').respond(mockData.tasks.GET);
  }
]);